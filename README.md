...
This repo contains the documented developing of a fast address extraction tool that can crawl the physical address of a business from it's website.The tool uses a dual model architecture, both models being built on a DistilBert(uncased) backbone.  
The first one is a Classification model with one FC layer used for predicting the probability of a text sequence to be an adddress and the second is a NER model used for formatting the address in the way requested.  
I will explain the reasoning behind this architecture and also how the crawling process works.


## Considerations

1. We are dealing with addreses from multiple countries, in distinct formats.
...
6. All valid addreses contain a postal code so this is a good starting point  
...
The scraper is filtering the text tags by applying a POSTCODE regex pattern to each text element that contains digits.In this way, the tool extracts the text elements that can contain an address in it, covering all possible cases.Then it appends the found text elements to a list, alongside the found postal code and the probability of it being an address(calculated by inference on the Classification model).  
...
After that the scraper sorts the found addreses based on their probability and if the top address on the first page has a probability over 0.90 it doesn't search other pages on the respective website.If not, other relevant relative links(eg. 'Contact' or 'Imprint') are parsed in the same way and are added to a list to be sorted.  

## Formatting the address

The model has a good accuracy, but given the incredible variability we can't use only the first predicition in the list, so we have to make use of all of them.
...
The purpose is to train a NER model that not only formats clean addreses, but also addreses surrounded by text like `The Jewelry Store </br> Address: </br>Russell Road, Edinburgh EH11 2LS </br> Email : thejewelry.store@gmail.com`.(eg use case. if the bsoup splitter didn't work properly).  
...
3. Run 'crawler.py <file_path_containing_websites>'  
